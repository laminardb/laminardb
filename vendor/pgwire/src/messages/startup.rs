...
impl SslRequest {
    ...
    pub fn is_ssl_request_packet(buf: &[u8]) -> bool {
        if buf.remaining() >= Self::BODY_SIZE {
            let declared_len = (&buf[..4]).get_i32();
            let magic_code = (&buf[4..8]).get_i32();
            declared_len == Self::BODY_SIZE as i32 && magic_code == Self::BODY_MAGIC_NUMBER
        } else {
            false
        }
    }
...
impl Message for SASLInitialResponse {
    ...
    fn decode_body(
        ...
        let data = match data_len {
            -1 => None,
            0.. => Some(codec::take(buf, data_len as usize)?.freeze()),
            _ => {
                return Err(PgWireError::MalformedMessage(
                    "SASL initial response length must be -1 or nonnegative",
                ));
            }
        };
...
impl Message for NegotiateProtocolVersion {
    ...
    fn decode_body(
        ...
        for _ in 0..option_count {
            options.push(codec::get_cstring(buf)?.unwrap_or_default())
        }
