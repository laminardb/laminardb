...
// LaminarDB does not accept giant bound parameters or COPY frames. Keeping the
// protocol cap fixed also bounds codec buffering before handler admission.
pub(crate) const LARGE_PACKET_SIZE_LIMIT: usize = 8 * 1024 * 1024;
...
pub trait Message: Sized {
    ...
    fn encode(&self, buf: &mut BytesMut) -> PgWireResult<()> {
        ...
        let wire_len =
            i32::try_from(len).map_err(|_| PgWireError::MessageTooLarge(i32::MAX as usize, len))?;
        if wire_len < 4 {
            return Err(PgWireError::InvalidMessageLength(wire_len));
        }
        ...
        let result = (|| {
            ...
            let expected = len
                .checked_add(usize::from(Self::message_type().is_some()))
                .ok_or(PgWireError::MessageTooLarge(i32::MAX as usize, len))?;
            if buf.len() - start != expected {
                return Err(PgWireError::MalformedMessage(
                    "encoded body does not match declared message length",
                ));
            }
            Ok(())
        })();
...
mod test {
    ...
    #[test]
    fn encode_error_leaves_destination_unchanged() {
        ...
        impl Message for FailingMessage {
            ...
            fn encode_body(&self, buf: &mut BytesMut) -> PgWireResult<()> {
                buf.put_u8(1);
                Err(PgWireError::MalformedMessage("injected encode failure"))
            }
        ...
        let error = FailingMessage.encode(&mut buffer).unwrap_err();
