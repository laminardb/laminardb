use bytes::{Buf, BufMut, Bytes};

use super::{DecodeContext, Message, codec};
use crate::error::PgWireResult;

/// Request from frontend to parse a prepared query string
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Parse {
    pub name: Option<String>,
    pub query: String,
    pub type_oids: Vec<u32>,
}

/// Message type byte for Parse
pub const MESSAGE_TYPE_BYTE_PARSE: u8 = b'P';

impl Message for Parse {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_PARSE)
    }

    #[inline]
    fn max_message_length() -> usize {
        super::LARGE_PACKET_SIZE_LIMIT
    }

    fn message_length(&self) -> usize {
        4 + codec::option_string_len(&self.name) // name
            + (1 + self.query.len()) // query
            + 2 + (4 * self.type_oids.len()) // type oids
    }

    fn encode_body(&self, buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        let type_oid_count = u16::try_from(self.type_oids.len()).map_err(|_| {
            crate::error::PgWireError::MalformedMessage("Parse has too many parameter type OIDs")
        })?;
        codec::put_option_cstring(buf, &self.name);
        codec::put_cstring(buf, &self.query);

        buf.put_u16(type_oid_count);
        for oid in &self.type_oids {
            buf.put_u32(*oid);
        }

        Ok(())
    }

    fn decode_body(
        buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        let name = codec::get_cstring(buf)?;
        let query = codec::get_cstring(buf)?.unwrap_or_default();
        let type_oid_count = codec::get_u16(buf)?;
        if usize::from(type_oid_count) > buf.remaining() / 4 {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Parse type OID count exceeds message body",
            ));
        }

        let mut type_oids = Vec::with_capacity(type_oid_count as usize);
        for _ in 0..type_oid_count {
            type_oids.push(codec::get_u32(buf)?);
        }

        Ok(Parse {
            name,
            query,
            type_oids,
        })
    }
}

/// Response for Parse command, sent from backend to frontend
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct ParseComplete;

/// Message type byte for ParseComplete
pub const MESSAGE_TYPE_BYTE_PARSE_COMPLETE: u8 = b'1';

impl Message for ParseComplete {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_PARSE_COMPLETE)
    }

    #[inline]
    fn max_message_length() -> usize {
        super::SMALL_BACKEND_PACKET_SIZE_LIMIT
    }

    #[inline]
    fn message_length(&self) -> usize {
        4
    }

    #[inline]
    fn encode_body(&self, _buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        Ok(())
    }

    #[inline]
    fn decode_body(
        _buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        Ok(ParseComplete)
    }
}

/// Closing the prepared statement or portal
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Close {
    pub target_type: u8,
    pub name: Option<String>,
}

/// Target type byte for prepared statement
pub const TARGET_TYPE_BYTE_STATEMENT: u8 = b'S';
/// Target type byte for portal
pub const TARGET_TYPE_BYTE_PORTAL: u8 = b'P';

/// Message type byte for Close
pub const MESSAGE_TYPE_BYTE_CLOSE: u8 = b'C';

impl Message for Close {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_CLOSE)
    }

    fn message_length(&self) -> usize {
        4 + 1 + codec::option_string_len(&self.name)
    }

    fn encode_body(&self, buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        buf.put_u8(self.target_type);
        codec::put_option_cstring(buf, &self.name);
        Ok(())
    }

    fn decode_body(
        buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        let target_type = codec::get_u8(buf)?;
        let name = codec::get_cstring(buf)?;

        Ok(Close { target_type, name })
    }
}

/// Response for Close command, sent from backend to frontend
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct CloseComplete;

/// Message type byte for CloseComplete
pub const MESSAGE_TYPE_BYTE_CLOSE_COMPLETE: u8 = b'3';

impl Message for CloseComplete {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_CLOSE_COMPLETE)
    }

    #[inline]
    fn max_message_length() -> usize {
        super::SMALL_BACKEND_PACKET_SIZE_LIMIT
    }

    #[inline]
    fn message_length(&self) -> usize {
        4
    }

    #[inline]
    fn encode_body(&self, _buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        Ok(())
    }

    #[inline]
    fn decode_body(
        _buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        Ok(CloseComplete)
    }
}

/// Bind command, for executing prepared statement
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Bind {
    pub portal_name: Option<String>,
    pub statement_name: Option<String>,
    pub parameter_format_codes: Vec<i16>,
    // None for Null data, TODO: consider wrapping this together with DataRow in
    // data.rs
    pub parameters: Vec<Option<Bytes>>,

    pub result_column_format_codes: Vec<i16>,
}

/// Message type byte for Bind
pub const MESSAGE_TYPE_BYTE_BIND: u8 = b'B';

impl Message for Bind {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_BIND)
    }

    #[inline]
    fn max_message_length() -> usize {
        super::LARGE_PACKET_SIZE_LIMIT
    }

    fn message_length(&self) -> usize {
        4 + codec::option_string_len(&self.portal_name) + codec::option_string_len(&self.statement_name)
            + 2 // parameter_format_code len
            + (2 * self.parameter_format_codes.len()) // parameter_format_codes
            + 2 // parameters len
            + self.parameters.iter().map(|p| 4 + p.as_ref().map(|data| data.len()).unwrap_or(0)).sum::<usize>() // parameters
            + 2 // result_format_code len
            + (2 * self.result_column_format_codes.len()) // result_format_codes
    }

    fn encode_body(&self, buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        let parameter_format_count =
            u16::try_from(self.parameter_format_codes.len()).map_err(|_| {
                crate::error::PgWireError::MalformedMessage(
                    "Bind has too many parameter format codes",
                )
            })?;
        let parameter_count = u16::try_from(self.parameters.len()).map_err(|_| {
            crate::error::PgWireError::MalformedMessage("Bind has too many parameters")
        })?;
        let result_format_count =
            u16::try_from(self.result_column_format_codes.len()).map_err(|_| {
                crate::error::PgWireError::MalformedMessage("Bind has too many result format codes")
            })?;
        if self
            .parameter_format_codes
            .iter()
            .chain(&self.result_column_format_codes)
            .any(|code| !matches!(code, 0 | 1))
        {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind format code is not text or binary",
            ));
        }
        if !matches!(self.parameter_format_codes.len(), 0 | 1)
            && self.parameter_format_codes.len() != self.parameters.len()
        {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind parameter format count does not match parameter count",
            ));
        }
        if self
            .parameters
            .iter()
            .flatten()
            .any(|value| value.len() > i32::MAX as usize)
        {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind parameter exceeds protocol length",
            ));
        }

        codec::put_option_cstring(buf, &self.portal_name);
        codec::put_option_cstring(buf, &self.statement_name);

        buf.put_u16(parameter_format_count);
        for c in &self.parameter_format_codes {
            buf.put_i16(*c);
        }

        buf.put_u16(parameter_count);
        for v in &self.parameters {
            if let Some(v) = v {
                buf.put_i32(v.len() as i32);
                buf.put_slice(v.as_ref());
            } else {
                buf.put_i32(-1);
            }
        }

        buf.put_u16(result_format_count);
        for c in &self.result_column_format_codes {
            buf.put_i16(*c);
        }

        Ok(())
    }

    fn decode_body(
        buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        let portal_name = codec::get_cstring(buf)?;
        let statement_name = codec::get_cstring(buf)?;

        let parameter_format_code_len = codec::get_u16(buf)?;
        if usize::from(parameter_format_code_len) > buf.remaining() / 2 {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind parameter format count exceeds message body",
            ));
        }
        let mut parameter_format_codes = Vec::with_capacity(parameter_format_code_len as usize);

        for _ in 0..parameter_format_code_len {
            let code = codec::get_i16(buf)?;
            if !matches!(code, 0 | 1) {
                return Err(crate::error::PgWireError::MalformedMessage(
                    "Bind parameter format code is not text or binary",
                ));
            }
            parameter_format_codes.push(code);
        }

        let parameter_len = codec::get_u16(buf)?;
        if usize::from(parameter_len) > buf.remaining() / 4 {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind parameter count exceeds message body",
            ));
        }
        let mut parameters = Vec::with_capacity(parameter_len as usize);
        for _ in 0..parameter_len {
            let data_len = codec::get_i32(buf)?;

            match data_len {
                -1 => parameters.push(None),
                0.. => parameters.push(Some(codec::take(buf, data_len as usize)?.freeze())),
                _ => {
                    return Err(crate::error::PgWireError::MalformedMessage(
                        "Bind parameter length must be -1 or nonnegative",
                    ));
                }
            }
        }
        if !matches!(parameter_format_codes.len(), 0 | 1)
            && parameter_format_codes.len() != parameters.len()
        {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind parameter format count does not match parameter count",
            ));
        }

        let result_column_format_code_len = codec::get_u16(buf)?;
        if usize::from(result_column_format_code_len) > buf.remaining() / 2 {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Bind result format count exceeds message body",
            ));
        }
        let mut result_column_format_codes =
            Vec::with_capacity(result_column_format_code_len as usize);
        for _ in 0..result_column_format_code_len {
            let code = codec::get_i16(buf)?;
            if !matches!(code, 0 | 1) {
                return Err(crate::error::PgWireError::MalformedMessage(
                    "Bind result format code is not text or binary",
                ));
            }
            result_column_format_codes.push(code);
        }

        Ok(Bind {
            portal_name,
            statement_name,

            parameter_format_codes,
            parameters,

            result_column_format_codes,
        })
    }
}

/// Success response for `Bind`
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct BindComplete;

/// Message type byte for BindComplete
pub const MESSAGE_TYPE_BYTE_BIND_COMPLETE: u8 = b'2';

impl Message for BindComplete {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_BIND_COMPLETE)
    }

    #[inline]
    fn max_message_length() -> usize {
        super::SMALL_BACKEND_PACKET_SIZE_LIMIT
    }

    #[inline]
    fn message_length(&self) -> usize {
        4
    }

    #[inline]
    fn encode_body(&self, _buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        Ok(())
    }

    #[inline]
    fn decode_body(
        _buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        Ok(BindComplete)
    }
}

/// Describe command fron frontend to backend. For getting information of
/// particular portal or statement
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Describe {
    pub target_type: u8,
    pub name: Option<String>,
}

/// Message type byte for Describe
pub const MESSAGE_TYPE_BYTE_DESCRIBE: u8 = b'D';

impl Message for Describe {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_DESCRIBE)
    }

    fn message_length(&self) -> usize {
        4 + 1 + codec::option_string_len(&self.name)
    }

    fn encode_body(&self, buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        buf.put_u8(self.target_type);
        codec::put_option_cstring(buf, &self.name);
        Ok(())
    }

    fn decode_body(
        buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        let target_type = codec::get_u8(buf)?;
        let name = codec::get_cstring(buf)?;

        Ok(Describe { target_type, name })
    }
}

/// Execute portal by its name
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Execute {
    pub name: Option<String>,
    pub max_rows: i32,
}

/// Message type byte for Execute
pub const MESSAGE_TYPE_BYTE_EXECUTE: u8 = b'E';

impl Message for Execute {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_EXECUTE)
    }

    fn message_length(&self) -> usize {
        4 + codec::option_string_len(&self.name) + 4
    }

    fn encode_body(&self, buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        if self.max_rows < 0 {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Execute row count must be nonnegative",
            ));
        }
        codec::put_option_cstring(buf, &self.name);
        buf.put_i32(self.max_rows);
        Ok(())
    }

    fn decode_body(
        buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        let name = codec::get_cstring(buf)?;
        let max_rows = codec::get_i32(buf)?;
        if max_rows < 0 {
            return Err(crate::error::PgWireError::MalformedMessage(
                "Execute row count must be nonnegative",
            ));
        }

        Ok(Execute { name, max_rows })
    }
}

/// Flush message to force backend to deliver any pending output
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Flush;

/// Message type byte for Flush
pub const MESSAGE_TYPE_BYTE_FLUSH: u8 = b'H';

impl Message for Flush {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_FLUSH)
    }

    #[inline]
    fn message_length(&self) -> usize {
        4
    }

    fn encode_body(&self, _buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        Ok(())
    }

    fn decode_body(
        _buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        Ok(Flush)
    }
}

/// Execute portal by its name
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct Sync;

/// Message type byte for Sync
pub const MESSAGE_TYPE_BYTE_SYNC: u8 = b'S';

impl Message for Sync {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_SYNC)
    }

    #[inline]
    fn message_length(&self) -> usize {
        4
    }

    fn encode_body(&self, _buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        Ok(())
    }

    fn decode_body(
        _buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        Ok(Sync)
    }
}

/// Indicates a portal execution has been suspended due to reaching max rows
#[non_exhaustive]
#[derive(PartialEq, Eq, Debug, new)]
pub struct PortalSuspended;

/// Message type byte for PortalSuspended
pub const MESSAGE_TYPE_BYTE_PORTAL_SUSPENDED: u8 = b's';

impl Message for PortalSuspended {
    #[inline]
    fn message_type() -> Option<u8> {
        Some(MESSAGE_TYPE_BYTE_PORTAL_SUSPENDED)
    }

    #[inline]
    fn max_message_length() -> usize {
        super::SMALL_BACKEND_PACKET_SIZE_LIMIT
    }

    #[inline]
    fn message_length(&self) -> usize {
        4
    }

    fn encode_body(&self, _buf: &mut bytes::BytesMut) -> PgWireResult<()> {
        Ok(())
    }

    fn decode_body(
        _buf: &mut bytes::BytesMut,
        _: usize,
        _ctx: &DecodeContext,
    ) -> PgWireResult<Self> {
        Ok(PortalSuspended)
    }
}

#[cfg(test)]
mod tests {
    use bytes::{BufMut, BytesMut};

    use super::{Bind, Execute};
    use crate::error::PgWireError;
    use crate::messages::{DecodeContext, Message};

    #[test]
    fn bind_rejects_negative_result_format_count_without_allocating() {
        let mut packet = BytesMut::new();
        packet.put_u8(b'B');
        packet.put_i32(12);
        packet.put_u8(0);
        packet.put_u8(0);
        packet.put_u16(0);
        packet.put_u16(0);
        packet.put_i16(-1);

        let error = Bind::decode(&mut packet, &DecodeContext::default()).unwrap_err();
        assert!(matches!(error, PgWireError::MalformedMessage(_)));
    }

    #[test]
    fn malformed_bind_cannot_consume_the_following_frame() {
        let mut packet = BytesMut::new();
        packet.put_u8(b'B');
        packet.put_i32(14);
        packet.put_u8(0);
        packet.put_u8(0);
        packet.put_u16(0);
        packet.put_u16(1);
        packet.put_i32(100);
        packet.put_u8(b'S');
        packet.put_i32(4);

        let error = Bind::decode(&mut packet, &DecodeContext::default()).unwrap_err();
        assert!(matches!(error, PgWireError::MalformedMessage(_)));
        assert_eq!(&packet[..], &[b'S', 0, 0, 0, 4]);
    }

    #[test]
    fn execute_rejects_negative_row_count() {
        let mut packet = BytesMut::new();
        packet.put_u8(b'E');
        packet.put_i32(9);
        packet.put_u8(0);
        packet.put_i32(-1);

        let error = Execute::decode(&mut packet, &DecodeContext::default()).unwrap_err();
        assert!(matches!(error, PgWireError::MalformedMessage(_)));
    }

    #[test]
    fn oversized_bind_count_encode_is_transactional() {
        let bind = Bind::new(
            None,
            None,
            vec![0; usize::from(u16::MAX) + 1],
            vec![],
            vec![],
        );
        let mut packet = BytesMut::from(&b"prefix"[..]);

        let error = bind.encode(&mut packet).unwrap_err();

        assert!(matches!(error, PgWireError::MalformedMessage(_)));
        assert_eq!(&packet[..], b"prefix");
    }

    #[test]
    fn negative_execute_encode_is_transactional() {
        let execute = Execute::new(None, -1);
        let mut packet = BytesMut::from(&b"prefix"[..]);

        let error = execute.encode(&mut packet).unwrap_err();

        assert!(matches!(error, PgWireError::MalformedMessage(_)));
        assert_eq!(&packet[..], b"prefix");
    }
}
