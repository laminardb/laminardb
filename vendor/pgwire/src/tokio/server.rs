    ...
    S: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'static,
{
    ...
    // PostgreSQL terminates a FATAL exchange after ErrorResponse. Sending
    // ReadyForQuery here is both misleading during startup and observable as
    // a protocol violation by strict clients.
    if is_fatal {
        socket.flush().await?;
    ...
    ($socket:expr, $startup_timeout:expr, $handlers:expr) => {{
        ...
        loop {
            ...
            match msg {
                ...
                Some(Err(mut e)) => {
                    // Decoder failures are protocol errors, not silent EOFs.
                    // Route them through the configured hook exactly once,
                    // return a FATAL ErrorResponse, and terminate the socket.
                    error_handler.on_error(socket, &mut e);
...
mod protocol_error_tests {
    ...
    impl PgWireServerHandlers for TestHandlers {
        fn error_handler(&self) -> Arc<impl ErrorHandler> {
            Arc::new(CountingErrorHandler(Arc::clone(&self.errors)))
        }
    }

    #[tokio::test]
    async fn malformed_startup_frame_reaches_error_handler_once() {
        ...
        // Eight bytes let SSL classification finish; the impossible declared
        // startup length is then rejected by the normal startup decoder.
        client
            .write_all(&[0, 0, 0, 3, 0, 0, 0, 0])
            .await
            .expect("write malformed startup");

        assert_eq!(client.read_u8().await.expect("response type"), b'E');
        let length = client.read_i32().await.expect("response length");
        assert!(length >= 4);
        let mut body = vec![0; usize::try_from(length - 4).expect("body length")];
        client.read_exact(&mut body).await.expect("error body");
        assert!(
            body.windows(5).any(|window| window == b"08P01"),
            "expected protocol-violation SQLSTATE in {body:?}"
        );

        server.await.expect("server task").expect("process socket");
