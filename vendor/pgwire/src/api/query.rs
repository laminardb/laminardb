...
pub trait SimpleQueryHandler: Send + Sync {
        ...
        PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
    {
        ...
        // PostgreSQL destroys the unnamed prepared statement and portal when
        // a simple Query message begins.
        ...
        if !matches!(client.state(), PgWireConnectionState::CopyInProgress(_)) {
            ...
            if matches!(transaction_status, TransactionStatus::Idle) {
...
pub trait ExtendedQueryHandler: Send + Sync {
        ...
        PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
    {
        ...
        let result = if let Some(cancel_rx) = cancel_rx {
            match select(self._on_execute(client, message), cancel_rx).await {
                Either::Left((result, _)) => result,
        ...
        PgWireError: From<<C as Sink<PgWireBackendMessage>>::Error>,
    {
        ...
        let max_rows = usize::try_from(message.max_rows)
            .map_err(|_| PgWireError::MalformedMessage("Execute row count must be nonnegative"))?;
        ...
        let needs_fetch = if matches!(
            ...
            let resp = self.do_query(client, portal.as_ref(), max_rows).await?;
        ...
        if needs_fetch {
            loop {
                ...
                if fetch_result.suspended && max_rows == 0 {
                    client.flush().await?;
                    continue;
                }
                if fetch_result.suspended {
                    ...
                    let tag = Tag::new(&command_tag).with_rows(fetch_result.total_rows);
                    client
