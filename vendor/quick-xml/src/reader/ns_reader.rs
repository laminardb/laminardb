...
#[cfg(test)]
mod security_backport_tests {
    use super::*;
    use crate::name::{NamespaceError, DEFAULT_MAX_DECLARATIONS_PER_ELEMENT};

    fn document(count: usize) -> String {
        let declarations: String = (0..count)
            .map(|i| format!(" xmlns:p{i}=\"urn:{i}\""))
            .collect();
        format!("<root{declarations}><value>ok</value></root>")
    }

    #[test]
    fn namespace_limit_reaches_slice_and_buffered_readers() {
        let limit = DEFAULT_MAX_DECLARATIONS_PER_ELEMENT;
        let valid = document(limit);
        assert!(matches!(NsReader::from_str(&valid).read_event(), Ok(Event::Start(_))));
        let invalid = document(limit + 1);
        assert!(matches!(
            NsReader::from_str(&invalid).read_event(),
            Err(crate::Error::Namespace(NamespaceError::TooManyDeclarations(n))) if n == limit
        ));
        assert!(matches!(
            NsReader::from_reader(invalid.as_bytes()).read_event_into(&mut Vec::new()),
            Err(crate::Error::Namespace(NamespaceError::TooManyDeclarations(n))) if n == limit
        ));
    }
    ...
    #[test]
    fn cloud_response_deserialization_enforces_namespace_limit() {
        #[derive(Debug, serde_derive::Deserialize)]
        struct Document {
            value: String,
        }
        let valid = document(DEFAULT_MAX_DECLARATIONS_PER_ELEMENT);
        assert_eq!(crate::de::from_str::<Document>(&valid).unwrap().value, "ok");
        let invalid = document(DEFAULT_MAX_DECLARATIONS_PER_ELEMENT + 1);
        let error = crate::de::from_str::<Document>(&invalid).unwrap_err();
        assert!(matches!(
            error,
            crate::DeError::InvalidXml(crate::Error::Namespace(NamespaceError::TooManyDeclarations(_)))
        ));
    }
}
