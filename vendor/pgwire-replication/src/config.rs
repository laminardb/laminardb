...
pub struct ReplicationConfig {
    ...
    /// Exact PostgreSQL recovery identity required before streaming starts.
    ///
    /// When present, the replication connection validates this identity with
    /// `IDENTIFY_SYSTEM` and verifies the slot cursor on the same authenticated
    /// socket before issuing `START_REPLICATION`.
    pub expected_recovery_identity: Option<ExpectedRecoveryIdentity>,
    ...
    /// Maximum payload bytes accepted for one PostgreSQL backend message.
    ///
    /// The limit is checked before allocating the payload buffer. This is a
    ...
    /// event channel, and held by the consumer. The event-count limit remains
    /// a secondary bound for zero/small-payload control events. The connection
    /// also owns one fixed 128 KiB socket read-ahead buffer outside this limit.
...
/// PostgreSQL cluster incarnation expected by a durable recovery checkpoint.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ExpectedRecoveryIdentity {
    /// Physical PostgreSQL cluster identifier.
    pub system_identifier: u64,
    /// Exact WAL timeline admitted by the checkpoint.
    pub timeline_id: u32,
}
