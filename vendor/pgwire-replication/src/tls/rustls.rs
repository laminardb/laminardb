...
pub async fn maybe_upgrade_to_tls(
    ...
    let cfg = build_rustls_config(tls, verify_chain, verify_hostname)?;
...
mod tests {
    ...
    fn mtls_requires_both_cert_and_key() {
        ...
        let err = build_rustls_config(&tls, false, false).unwrap_err();
    ...
    #[test]
    fn verify_full_accepts_ip_server_names_for_ip_san_validation() {
        let server_name = rustls::pki_types::ServerName::try_from("192.168.1.1".to_string())
            .expect("IP literals are valid rustls server names");
        assert!(matches!(
            server_name,
