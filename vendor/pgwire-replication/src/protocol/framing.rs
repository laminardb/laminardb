...
pub async fn read_backend_message_with_limit<R: AsyncRead + Unpin>(
    rd: &mut R,
    max_message_size: usize,
) -> Result<BackendMessage> {
    let mut reader = MessageReader::with_capacity_and_limit(4096, max_message_size);
    reader.read(rd).await
}
...
impl MessageReader {
    ...
    pub fn with_capacity(capacity: usize) -> Self {
        Self::with_capacity_and_limit(capacity, MAX_MESSAGE_SIZE)
    }

    pub fn with_capacity_and_limit(capacity: usize, max_message_size: usize) -> Self {
        Self {
            hdr: [0u8; 5],
            hdr_filled: 0,
            payload: Vec::with_capacity(capacity.min(max_message_size)),
            payload_filled: 0,
            payload_len: None,
            tag: 0,
            max_message_size,
    ...
    pub async fn read<R: AsyncRead + Unpin>(&mut self, rd: &mut R) -> Result<BackendMessage> {
        let (message, permit) = self.read_inner(rd, None).await?;
        debug_assert!(permit.is_none());
        Ok(message)
    }

    /// Reads one message after reserving its declared payload bytes.
    ///
    /// The reservation happens after the fixed header is validated and before
    /// the payload buffer is allocated. It is returned with the message so the
    /// caller can retain it for as long as any slice of that allocation lives.
    pub(crate) async fn read_with_budget<R: AsyncRead + Unpin>(
        &mut self,
        rd: &mut R,
        byte_budget: &Arc<Semaphore>,
    ) -> Result<(BackendMessage, OwnedSemaphorePermit)> {
        let (message, permit) = self.read_inner(rd, Some(byte_budget)).await?;
        let permit = permit.ok_or_else(|| {
            PgWireError::Internal("budgeted message read completed without a reservation".into())
        })?;
        Ok((message, permit))
    }

    async fn read_inner<R: AsyncRead + Unpin>(
        &mut self,
        rd: &mut R,
        byte_budget: Option<&Arc<Semaphore>>,
    ) -> Result<(BackendMessage, Option<OwnedSemaphorePermit>)> {
        ...
        if let Some(byte_budget) = byte_budget {
            if self.payload_permit.is_none() {
                let permits = u32::try_from(payload_len).map_err(|_| {
                    PgWireError::Protocol(format!(
                        "backend message payload cannot be byte-budgeted: {payload_len} bytes"
                    ))
                })?;
                    ...
                    Arc::clone(byte_budget)
                        ...
                        .map_err(|_| {
                            PgWireError::Internal(
                                "backend message byte budget closed unexpectedly".into(),
                            )
                        })?,
                );
            }
        ...
        Ok((BackendMessage { tag, payload }, permit))
    }
}
...
pub async fn read_backend_message_into<R: AsyncRead + Unpin>(
    ...
    read_backend_message_into_with_limit(rd, buf, MAX_MESSAGE_SIZE).await
}

pub async fn read_backend_message_into_with_limit<R: AsyncRead + Unpin>(
    rd: &mut R,
    buf: &mut BytesMut,
    max_message_size: usize,
) -> Result<BackendMessage> {
...
mod tests {
    ...
    #[tokio::test]
    async fn message_reader_rejects_configured_limit_before_allocating_payload() {
        let data = [b'd', 0, 0, 0, 13];
        let mut cursor = Cursor::new(&data[..]);
        let mut reader = MessageReader::with_capacity_and_limit(0, 8);

        let err = reader.read(&mut cursor).await.unwrap_err();

        assert!(err.to_string().contains("9 bytes (max 8)"));
        assert_eq!(reader.payload.capacity(), 0);
    }

    #[tokio::test]
    async fn message_reader_reserves_aggregate_bytes_before_payload_allocation() {
        let data = [b'd', 0, 0, 0, 8, 1, 2, 3, 4, b'd', 0, 0, 0, 8, 5, 6, 7, 8];
        let mut cursor = Cursor::new(&data[..]);
        let budget = Arc::new(Semaphore::new(7));
        let mut reader = MessageReader::with_capacity_and_limit(0, 8);

        let (first, first_permit) = reader.read_with_budget(&mut cursor, &budget).await.unwrap();
        assert_eq!(&first.payload[..], &[1, 2, 3, 4]);
        assert_eq!(budget.available_permits(), 3);

        let second = reader.read_with_budget(&mut cursor, &budget);
        tokio::pin!(second);
        assert!(
            tokio::time::timeout(std::time::Duration::from_millis(20), &mut second)
                .await
                .is_err(),
            "the second payload must wait for aggregate byte capacity"
        );
        drop(first);
        drop(first_permit);
        let (second, second_permit) = second.await.unwrap();
        assert_eq!(&second.payload[..], &[5, 6, 7, 8]);
        assert_eq!(budget.available_permits(), 3);
        drop(second_permit);
