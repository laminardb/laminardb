...
The `StateStore` trait is the single most critical abstraction in the Delta Architecture. Every stateful operator (windows, joins, aggregations, materialized views) accesses its partition's state through this trait. Because it runs on the Ring 0 hot path:
