...
- Support both embedded mode (single-node, in-process) and delta mode (multi-node, distributed)
...
- Configuration distribution across delta nodes (each node reads its own file)
...
fn validate_config(config: &ServerConfig) -> Result<(), ConfigError> {
    ...
    if config.server.mode == "delta" {
        if config.discovery.is_none() {
            errors.push(
                "mode = \"delta\" requires a [discovery] section".to_string()
            );
        }
        if config.coordination.is_none() {
            errors.push(
                "mode = \"delta\" requires a [coordination] section".to_string()
            );
        }
        if config.node_id.is_none() {
            errors.push(
                "mode = \"delta\" requires node_id to be set".to_string()
            );
        }
    }
...
pub struct ServerConfig {
    ...
    /// Delta mode settings (optional; absent = embedded mode).
...
2. load_config(path) is called:
   ...
   d. Validate referential integrity:
      ...
      - Delta fields present if mode == "delta"
...
```toml
# laminardb-delta.toml -- Multi-node delta mode
...
- [ ] `test_parse_full_delta_config` -- Complete delta mode config
...
- [ ] `test_delta_mode_requires_discovery` -- Validation error if mode=delta without [discovery]
...
- [ ] Should delta mode config be in the same file or a separate overlay file?
...
- [ ] Example TOML files for embedded and delta modes
- [ ] Wired into `main.rs`
...
- [F-SERVER-005: Delta Mode](F-SERVER-005-delta-mode.md) -- Extended config for multi-node
