...
- Four named deployment profiles: `bare-metal`, `embedded`, `durable`, `delta`
...
| `delta` | `FxHashMap` | rkyv + object_store | redb | gRPC + gossip | Multi-node distributed deployment |
...
| `delta` | `delta` (enables durable + chitchat, openraft, tonic) |

The `delta` feature implies `durable`. The `durable` feature implies redb support.
...
| Capability | bare-metal | embedded | durable | delta |
|-----------|:---------:|:--------:|:-------:|:------------:|
| SQL queries | Y | Y | Y | Y |
...
| `delta` | durable validation + bind address available, seed nodes reachable |
...
| `FeatureNotCompiled` | Profile requires uncompiled feature (e.g., delta without `delta` feature) | Compile error or runtime error with clear message |
...
- [ ] `Profile::Delta` fails without `delta` feature
...
- [ ] Feature gate mapping (`durable`, `delta`) implemented
