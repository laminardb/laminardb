...
Discovery operates in **Ring 2 (Control Plane)**. It runs as a background tokio task, separate from the hot path. Membership changes are propagated to the `DeltaManager` via a `watch` channel, which then triggers partition rebalancing, Raft reconfiguration, or other control-plane actions.
...
/// Discovery trait: how nodes find each other in a delta.
///
...
/// Information about a node in the delta.
///
/// This is the canonical representation of a peer node, shared across
/// all discovery backends and the delta manager.
...
/// State of a node in the delta.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Serialize, Deserialize)]
pub enum NodeState {
    /// Node is joining the delta (Rising). Not yet assigned partitions.
    Joining,
    ...
    /// Node has left the delta. Its partitions have been reassigned.
    Left,
}
...
pub struct StaticDiscoveryConfig {
    ...
    /// Should include all nodes in the delta except this one.
...
5. **Membership Notification**: After each heartbeat round, the membership `watch` channel is updated with the current list of active peers. Consumers (e.g., `DeltaManager`) receive notifications of any changes.
