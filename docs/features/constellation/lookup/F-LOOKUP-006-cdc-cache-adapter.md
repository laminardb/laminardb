...
Three deployment fan-out strategies handle different constellation sizes:

1. **Embedded** (1 node): `Postgres -> CDC -> foyer` (direct connection)
2. **Constellation (3-5 nodes)**: `Postgres -> CDC -> foyer` per node (independent connections)
3. **Constellation (50+ nodes)**: `Postgres -> CDC -> Kafka compact topic -> foyer` (Kafka fan-out to avoid N connections to Postgres)
...
- Support Kafka compact topic fan-out for large constellations
...
pub struct CdcCacheAdapterConfig {
    ...
    /// Fan-out strategy for constellation mode.
...
```
How many nodes in the constellation?
