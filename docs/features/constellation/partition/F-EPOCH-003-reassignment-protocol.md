...
The migration protocol operates across **Ring 2 (Control Plane)** for coordination and **Ring 1 (Background)** for checkpoint I/O. The hot path (Ring 0) is stopped during migration. The protocol is orchestrated by the `ConstellationManager` which drives state transitions on both the old and new owner.
...
pub struct MigrationExecutor {
    ...
    /// Reference to the constellation metadata (Raft state).
    metadata: Arc<ConstellationMetadata>,
...
2. For each move, the `ConstellationManager` on the old owner node starts `execute_release()`.
...
8. New owner's `ConstellationManager` observes the release via Raft.
...
6. **Phase 6**: Integration tests with full constellation setup
