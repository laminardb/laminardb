...
+------------------+              +------------------------+
| Event Processing |              | ConstellationMetadata  |
|                  |              | (Raft State Machine)   |
| guard.check()   |              |                        |
...
/// Unique identifier for a partition within the constellation.
///
...
/// This constellation `PartitionId(u32)` is a DIFFERENT type — it identifies
/// a partition in the constellation's hash ring, not a source partition.
/// A mapping layer bridges the two: constellation PartitionId → set of
/// source PartitionIds that route to it.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
pub struct PartitionId(pub u32);

/// Unique identifier for a node (Star) within the constellation.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
...
impl PartitionGuard {
    ...
    /// be > 0 and must match the value committed in ConstellationMetadata.
    ...
    ///
    /// This performs a read from ConstellationMetadata, which may involve
    ...
    pub fn refresh(&self, metadata: &ConstellationMetadata) -> bool {
...
impl PartitionGuardSet {
    ...
    pub fn refresh_all(&self, metadata: &ConstellationMetadata) -> Vec<PartitionId> {
        self.guards
            .iter()
...
pub enum EpochError {
    ...
    /// The partition is not known to the constellation metadata.
...
1. **Guard Acquisition**: When Raft assigns partition P to node N at epoch E, the `ConstellationManager` creates `PartitionGuard::new(P, E, N)` and inserts it into the node's `PartitionGuardSet`.
...
| `EpochError::UnknownPartition` | Partition ID not found in constellation metadata | Log error; partition may have been deleted or never configured |
...
2. **Phase 2**: Integrate with `ConstellationMetadata` (depends on F-COORD-001)
...
- [ ] Should `PartitionGuard` implement `Drop` to automatically notify the constellation manager when a guard is released?
