...
- Define `ConstellationManager` as the top-level orchestrator for distributed LaminarDB
...
The `ConstellationManager` runs in **Ring 2 (Control Plane)** as a set of tokio tasks. It subscribes to events from the Discovery and Raft subsystems and drives actions in response. On the Raft leader, it additionally runs the partition assignment algorithm and initiates migrations.

```
+------------------------------------------------------------------+
|  ConstellationManager (Ring 2)                                    |
|                                                                   |
|  +---------------------+  +-------------------+  +-------------+ |
...
/// Configuration for the constellation manager.
...
pub struct ConstellationConfig {
    ...
    /// Number of partitions in the constellation.
    /// Must be set at constellation creation and cannot change.
...
/// The top-level constellation orchestrator.
///
/// Each node in a LaminarDB constellation runs exactly one
/// `ConstellationManager`. It coordinates discovery, Raft consensus,
...
pub struct ConstellationManager {
    ...
    state: Arc<RwLock<ConstellationState>>,

    /// Event channel for internal coordination.
    event_tx: mpsc::Sender<ConstellationEvent>,
    event_rx: mpsc::Receiver<ConstellationEvent>,

    /// Cancellation token for graceful shutdown.
    cancel: tokio_util::sync::CancellationToken,
}

/// The observable state of the constellation from this node's perspective.
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Lifecycle phases of a node in the constellation.
...
/// Internal events processed by the ConstellationManager event loop.
#[derive(Debug)]
enum ConstellationEvent {
    /// A new node joined the constellation (from discovery).
    NodeJoined(NodeInfo),
...
impl ConstellationManager {
    ...
    pub async fn new(
        ...
        Ok(Self {
            ...
            migration,
            guard_set,
            state: Arc::new(RwLock::new(ConstellationState {
    ...
    /// Start the constellation manager.
    ///
    ...
    pub async fn start(&mut self) -> Result<(), ConstellationError> {
        // Phase 1: Discover peers
        self.update_phase(NodeLifecyclePhase::Discovering).await;
        self.discovery.start().await
            .map_err(|e| ConstellationError::DiscoveryFailed(e.to_string()))?;
    ...
    /// 5. Leave discovery
    pub async fn shutdown(&self) -> Result<(), ConstellationError> {
        ...
        self.event_tx.send(ConstellationEvent::ManualRebalance).await
            .map_err(|_| ConstellationError::ChannelClosed)?;
    ...
    async fn wait_for_peers(&self) -> Result<Vec<NodeInfo>, ConstellationError> {
        ...
        loop {
            ...
            if start.elapsed() > timeout {
                return Err(ConstellationError::NoPeersFound);
            }
    ...
    async fn wait_for_assignment(&self) -> Result<Vec<PartitionId>, ConstellationError> {
        if self.raft.is_leader().await {
            ...
            ).map_err(|e| ConstellationError::AssignmentFailed(format!("{:?}", e)))?;
        ...
        loop {
            ...
            if start.elapsed() > timeout {
                return Err(ConstellationError::AssignmentTimeout);
            }
        ...
        for pid in partitions {
            ...
            if let Some(ckpt) = checkpoint {
                ...
                self.migration.restore_partition(*pid, &ckpt).await
                    .map_err(|e| ConstellationError::RestoreFailed {
                        partition: *pid,
                        source: format!("{}", e),
                    })?;
            }
    ...
    async fn handle_node_join(&self, node: NodeInfo) -> Result<(), ConstellationError> {
        if !self.raft.is_leader().await {
    ...
    async fn handle_node_failure(&self, node_id: NodeId) -> Result<(), ConstellationError> {
        if !self.raft.is_leader().await {
            return Ok(());
        }
    ...
    async fn wait_for_partition_drain(&self) -> Result<(), ConstellationError> {
        ...
        loop {
            ...
            if start.elapsed() > timeout {
                return Err(ConstellationError::DrainTimeout {
    ...
    fn spawn_event_loop(&self) {
        // Process ConstellationEvents from the channel
    ...
    /// Get the current constellation state (for monitoring/admin API).
    pub async fn state(&self) -> ConstellationState {
...
/// Errors from the constellation manager.
#[derive(Debug, Clone)]
...
impl From<CoordinationError> for ConstellationError {
    fn from(e: CoordinationError) -> Self {
        ConstellationError::CoordinationError(e)
    }
}
...
impl fmt::Display for ConstellationError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ConstellationError::DiscoveryFailed(s) => write!(f, "discovery failed: {}", s),
            ConstellationError::NoPeersFound => write!(f, "no peers found"),
            ConstellationError::RaftFormationFailed(s) => write!(f, "raft formation failed: {}", s),
            ConstellationError::AssignmentFailed(s) => write!(f, "assignment failed: {}", s),
            ConstellationError::AssignmentTimeout => write!(f, "assignment timeout"),
            ConstellationError::RestoreFailed { partition, source } => {
                write!(f, "restore failed for {:?}: {}", partition, source)
            }
            ConstellationError::DrainTimeout { remaining_partitions } => {
                write!(f, "drain timeout: {} partitions remaining", remaining_partitions)
            }
            ConstellationError::CoordinationError(e) => write!(f, "coordination: {}", e),
            ConstellationError::ChannelClosed => write!(f, "internal channel closed"),
        }
    }
}
...
1. `ConstellationManager::start()` is called.
...
2. Membership watcher emits `ConstellationEvent::NodeJoined`.
...
2. Membership watcher emits `ConstellationEvent::NodeSuspected`.
...
|-------|-------|----------|
| `ConstellationError::NoPeersFound` | Network isolation or misconfigured discovery | Check network, verify seed nodes/peer addresses |
| `ConstellationError::RaftFormationFailed` | Cannot reach quorum of peers for Raft | Ensure majority of nodes are reachable |
| `ConstellationError::AssignmentTimeout` | Leader not elected or assignment not committed | Wait for Raft leader election; check Raft health |
| `ConstellationError::RestoreFailed` | Checkpoint missing or corrupted | Alert operator; partition starts fresh (data loss) |
| `ConstellationError::DrainTimeout` | Migrations stuck during shutdown | Force shutdown; other nodes will recover via forced migration |
...
- [ ] `test_constellation_config_creation` - Valid config creates manager
...
- [ ] `test_constellation_error_display` - All error variants format correctly
...
1. **Phase 1**: Define `ConstellationManager`, `ConstellationConfig`, `ConstellationState`, and `ConstellationError`
...
- [ ] Should the constellation manager support "single-node mode" where no discovery or Raft is needed (for development/testing)?
...
- [ ] Should the constellation manager persist its state locally for faster restart, or always rebuild from Raft?
...
- [F-COORD-001: ConstellationMetadata & Raft Integration](F-COORD-001-raft-metadata.md)
