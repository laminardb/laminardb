...
/// Configuration for the constellation checkpoint benchmark.
#[derive(Debug, Clone)]
struct ConstellationCheckpointConfig {
...
impl ConstellationCheckpointConfig {
...
fn standard_configs() -> Vec<ConstellationCheckpointConfig> {
    vec![
        // 3 nodes, 50% capacity, 1GB state per node
        ConstellationCheckpointConfig {
            node_count: 3,
...
fn bench_constellation_checkpoint(c: &mut Criterion) {
    let mut group = c.benchmark_group("constellation_checkpoint");
                ...
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        let duration = rt.block_on(async {
                            run_constellation_checkpoint(&config).await
                        });
...
/// Run a single constellation checkpoint cycle and return the duration.
async fn run_constellation_checkpoint(
    config: &ConstellationCheckpointConfig,
) -> Duration {
...
criterion_group!(
    benches,
    bench_constellation_checkpoint,
...
|------------|-----------|-------|-----------------|-------------|
| 50% | 100MB | 3 | < 2s | `constellation_checkpoint/3node_50pct_100MB` |
| 80% | 100MB | 3 | < 5s | `constellation_checkpoint/3node_80pct_100MB` |
| 50% | 1GB | 3 | < 5s | `constellation_checkpoint/3node_50pct_1GB` |
| 80% | 1GB | 3 | < 10s | `constellation_checkpoint/3node_80pct_1GB` |
| 95% | 1GB | 3 | < 30s | `constellation_checkpoint/3node_95pct_1GB` |
...
- [ ] `test_benchmarks_compile` -- `cargo bench --bench constellation_checkpoint_bench --no-run` succeeds
...
2. **Phase 2**: Implement end-to-end constellation checkpoint benchmark
