...
```rust
use crate::constellation::rpc::proto::lookup_service_server::{
    ...
    async fn batch_lookup(
        ...
        let table = tables.get(&req.table_name)
            .ok_or_else(|| {
                ...
                crate::constellation::rpc::error_codes::table_not_found(&req.table_name)
            })?;

        // Validate epoch
        let current_epoch = table.epoch.load(std::sync::atomic::Ordering::Acquire);
        if req.epoch > 0 && req.epoch != current_epoch {
            return Err(crate::constellation::rpc::error_codes::epoch_mismatch(
                current_epoch,
                req.epoch,
            ));
        }
