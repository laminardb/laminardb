...
- **Status:** Frozen reader contract; admission-neutral and not wired to manifests
...
`VnodePartialV2` directory. It freezes a reader and test fixtures only. The current checkpoint
manifest has no selector for these bytes, so a production reader or writer must not sniff this
magic or fall back from this format to legacy rkyv. A manifest-version/format-selector change,
rolling reader capability proof, bounded object fetch, and complete restore transaction are later
admission blockers.
...
target belongs only to transition metadata. Assignment version and the assignment/operator/table
digests are nonzero. Exact stable
operator/table identity derivation remains a plan-contract blocker before a writer is enabled; the
format treats each as a plan-owned opaque 32-byte value and a reader compares it with its expected
value.
...
SUM when non-null count is zero. A zero non-null count evaluates to SQL NULL; otherwise SUM is its
exact signed `i64` value. `state_bytes == row_count * 24`, and scanned row, key, and state totals must
equal the header. The encoder consumes already sorted rows; sorting occurs on a bounded blocking
checkpoint worker or through an ordered LSM scan, never on the event-loop hot path.

## VnodePartialV2 directory

The current object-store provenance wrapper already uses `LDBVP2\0\0`; the inner directory
therefore uses distinct magic `LDBVPD\0\0`. Directory version is `2`, its header is 192 bytes, and
...
raw `(operator digest, table digest, vnode)` bytes. Both identity digests are nonzero and every
entry vnode equals the header vnode. A directory with only references has an empty body region and
uses `SHA256(empty)` as its body digest.
...
canonical parent and nonzero parent-entry digest. The selected inner decoder must verify that the
body envelope repeats the same attempt, assignment, identity, vnode, kind, and parent context.
...
across every BODY in that V2 object; a caller cannot reset them for each body. Directory metadata
uses the metadata and entry caps. Chain bytes and parent-link count accumulate across every fetched
REFERENCE/DELTA edge.

The borrowed decoder always receives an out-of-band expected context containing the source attempt
and fence, ABI/count/vnode, exact sorted plan roster, and per-entry expected envelope, codec,
routing schema, and contract. Passing internal hashes is not sufficient proof: after checked header
arithmetic and borrowed validation, every semantic field is compared with that cached context and
the trusted seal's top-level digest. SHA-256 cannot protect a deployment where an attacker may also
rewrite that trust root.
...
The borrowed codecs validate bytes already admitted under those reservations. They do not make the
current whole-object `read_partial` bounded, resolve chains, invoke operators, install state, alter
manifest dispatch, or relax `[LDB-4007]`. Whole-transition preflight must validate every roster
entry, aggregate the per-object counters, and finish every chain before any operator callback.

## Rolling compatibility

Release N may ship this reader and frozen goldens without a writer. A future N+1 writer is enabled
only after every checkpoint participant advertises the manifest-selected reader capability. Legacy
rkyv remains reachable only through explicit legacy inventory proof for the admitted global vnode-0
path. There is no magic-sniff fallback in either direction.
