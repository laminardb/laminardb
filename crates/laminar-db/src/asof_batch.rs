...
fn execute_asof_merge_scan(
    ...
    if left_timestamps.windows(2).any(|w| w[0] > w[1]) {
        return Err(DbError::Pipeline(
            "ASOF merge-scan requires sorted left timestamps".to_string(),
        ));
    }
    if right_timestamps.windows(2).any(|w| w[0] > w[1]) {
        return Err(DbError::Pipeline(
            "ASOF merge-scan requires sorted right timestamps".to_string(),
        ));
    }
