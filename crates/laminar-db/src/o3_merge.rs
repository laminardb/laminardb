...
//!
//! Inspired by QuestDB's O3 commit path, this module provides a buffer
...
#[derive(Debug)]
#[allow(dead_code)] // Used by O3MergeEngine which is pub(crate) but not yet wired into pipeline
        ...
        let concatenated = match concat_batches(&schema, &all_batches) {
            ...
            Err(e) => {
                tracing::warn!(
                    source = %source_name,
                    error = %e,
                    "O3 merge concat failed; returning on-time batches only"
                );
                // Return what we can — the late batches are lost.
                return all_batches;
            }
        };
...
fn sort_batch_by_column(batch: &RecordBatch, column_name: &str) -> Result<RecordBatch, String> {
    ...
    let indices = sort_to_indices(
        &sort_column.values,
        Some(sort_column.options.unwrap()),
        None,
    )
