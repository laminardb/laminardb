...
impl crate::pipeline::PipelineCallback for ConnectorPipelineCallback {
    ...
    fn extract_watermark(&mut self, source_idx: usize, batch: &RecordBatch) {
    ...
    fn filter_late_rows(&self, source_idx: usize, batch: &RecordBatch) -> Option<RecordBatch> {
    ...
    async fn maybe_checkpoint(
        ...
        let active_wm_count = self
            .watermark_contexts
            .iter()
            .flatten()
            .filter(|ctx| ctx.state.generator.current_watermark() > i64::MIN)
            .count();
        let mut per_source_watermarks = HashMap::with_capacity(active_wm_count);
