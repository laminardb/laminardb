        ...
        match msg {
            SourceMsg::Batch { source_idx, batch } => {
                ...
                if let Some(name) = self.source_names.get(source_idx) {
                    callback.extract_watermark(source_idx, &batch);
                    #[allow(clippy::cast_possible_truncation)]
                    {
                        *cycle_events += batch.num_rows() as u64;
                    }
                    if let Some(filtered) = callback.filter_late_rows(source_idx, &batch) {
...
mod tests {
    ...
    impl PipelineCallback for MockCallback {
        ...
        fn extract_watermark(&mut self, _source_idx: usize, batch: &RecordBatch) {
        ...
        fn filter_late_rows(&self, _source_idx: usize, batch: &RecordBatch) -> Option<RecordBatch> {
            Some(batch.clone())
        }
