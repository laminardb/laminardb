//! Constellation checkpoint benchmark (F-PERF-004)
//!
...
//! Run with:
//! ```text
//! cargo bench --bench constellation_checkpoint_bench -p laminar-core --features constellation
//! ```
...
// -- Constellation-gated imports -------------------------------------------

#[cfg(feature = "constellation")]
use laminar_core::constellation::checkpoint::distributed::{
    DistributedCheckpointConfig, DistributedCheckpointCoordinator,
};

#[cfg(feature = "constellation")]
use laminar_core::constellation::discovery::NodeId;

#[cfg(feature = "constellation")]
use laminar_core::constellation::partition::guard::{PartitionGuard, PartitionGuardSet};
...
use laminar_core::aggregation::gossip_aggregates::{
    AggregateState, GossipAggregateValue, merge_cluster_aggregates,
};
                    ...
                    for epoch in 0..10u64 {
                        ...
                        for node_idx in 1..=n {
                            let partitions: Vec<u32> =
                                (0..partitions_per_node).collect();
                            coord.record_prepare(
                                ckpt_id,
                                NodeId(node_idx),
                                partitions,
                            );
...
fn bench_partition_routing(c: &mut Criterion) {
    ...
    let multi_map =
        PartitionMap::from_assignments(256, &assignments, NodeId(1));
...
fn bench_aggregate_merge(c: &mut Criterion) {
    ...
    group.bench_function("merge_sum", |b| {
        let other = AggregateState::Sum(3.14);
        b.iter(|| {
...
fn bench_partition_guard(c: &mut Criterion) {
    ...
    for &partition_count in &[8u32, 32, 128] {
        ...
        group.bench_with_input(
            ...
            |b, set| {
                b.iter(|| {
                    black_box(set.check(black_box(0)))
                })
            },
        );
            ...
            |b, &n| {
                b.iter(|| {
                    ...
                    for node in &nodes {
                        for key in &keys {
                            black_box(
                                node.partition_map.partition_for_key(key),
                            );
                        }
                    }
                    ...
                    for node in &nodes {
                        let partitions: Vec<u32> =
                            (0..node.partition_count).collect();
                        coord.record_prepare(
                            ckpt_id,
                            node.node_id,
                            partitions,
...
// When the constellation feature is not enabled, provide a no-op bench
