...
async fn config_roundtrip_in_process_local_object_store() {
    ...
    let b = c.build().await.unwrap();
    b.write_partial(0, 1, Bytes::from_static(b"b")).await.unwrap();
    ...
    let b = c.build().await.unwrap();
    b.write_partial(0, 1, Bytes::from_static(b"c")).await.unwrap();
...
async fn distributed_embedded_static_two_instances_shared_store() {
    ...
    node_a.write_partial(0, 1, Bytes::from_static(b"A0")).await.unwrap();
    node_a.write_partial(1, 1, Bytes::from_static(b"A1")).await.unwrap();
    node_b.write_partial(2, 1, Bytes::from_static(b"B2")).await.unwrap();
    node_b.write_partial(3, 1, Bytes::from_static(b"B3")).await.unwrap();
    ...
    node_a.write_partial(0, 2, Bytes::from_static(b"A0@2")).await.unwrap();
