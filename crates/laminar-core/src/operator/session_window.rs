        ...
        + RkyvDeserialize<A::Acc, HighDeserializer<RkyvError>>,
{
    fn process(&mut self, event: &Event, ctx: &mut OperatorContext) -> OutputVec {
        ...
        match overlapping.len() {
            0 => {
                ...
                let session =
                    SessionMetadata::new(id, event_time, self.gap_ms);
    ...
    fn on_timer(&mut self, timer: Timer, ctx: &mut OperatorContext) -> OutputVec {
        ...
        let Some(session_id) =
            Self::session_id_from_timer(&timer.key)
        else {
            return output;
        };

        // Look up the pending timer entry: (expected_time, key_hash)
        let Some(&(expected_time, key_hash)) =
            self.pending_timers.get(&session_id.as_u64())
        else {
    ...
    fn checkpoint(&self) -> OperatorState {
        ...
        let checkpoint_data: (Vec<(u64, i64, u64)>, u64) =
            (timer_entries, counter_val);
    ...
    fn restore(&mut self, state: OperatorState) -> Result<(), OperatorError> {
        ...
        let archived = rkyv::access::<
            rkyv::Archived<(Vec<(u64, i64, u64)>, u64)>,
            RkyvError,
        >(&state.data)
        .map_err(|e| OperatorError::SerializationFailed(e.to_string()))?;
        let (timers, counter_val) = rkyv::deserialize::<
            (Vec<(u64, i64, u64)>, u64),
            RkyvError,
        >(archived)
        .map_err(|e| OperatorError::SerializationFailed(e.to_string()))?;
...
mod tests {
    ...
    fn first_pending_timer(
        op: &SessionWindowOperator<impl Aggregator>,
    ) -> (SessionId, i64, u64) {
        let (&sid_raw, &(time, kh)) =
            op.pending_timers.iter().next().expect("no pending timer");
    ...
    fn test_timer_key_roundtrip() {
        ...
        let timer_key =
            SessionWindowOperator::<CountAggregator>::timer_key(sid);
        let parsed =
            SessionWindowOperator::<CountAggregator>::session_id_from_timer(
                &timer_key,
            );
    ...
    fn test_session_late_event_side_output() {
        ...
        operator.set_late_data_config(LateDataConfig::with_side_output(
            "late".to_string(),
        ));
