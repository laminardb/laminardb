...
pub(crate) const TAG_BARRIER: u8 = 0x02;
...
pub(crate) async fn write_message<W>(writer: &mut W, msg: &ShuffleMessage) -> io::Result<()>
where
...
pub(crate) async fn read_message<R>(reader: &mut R) -> io::Result<ShuffleMessage>
where
...
mod tests {
    ...
    async fn sequenced_messages_preserve_order() {
        ...
        // order — the property per-key ordering relies on.
        let (mut a, mut b) = duplex(64 * 1024);
