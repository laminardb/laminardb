...
//!
//! Leverages the chitchat protocol (from Quickwit) for decentralized
...
impl GossipDiscovery {
    ...
    fn local_kvs(info: &NodeInfo) -> Vec<(String, String)> {
        let mut kvs = vec![
            ...
            (
                keys::LOAD_CORES.into(),
                info.metadata.cores.to_string(),
            ),
            (
...
mod tests {
    ...
    fn test_local_kvs() {
        ...
        assert!(kvs.iter().any(|(k, v)| k == keys::PARTITIONS_OWNED && v == "0,1,2"));
    }
