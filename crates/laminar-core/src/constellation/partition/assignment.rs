//! Partition assignment algorithms for the constellation.
//!
...
use crate::constellation::discovery::{NodeId, NodeInfo};
    ...
    fn validate_plan(
        ...
        for (pid, owner) in &plan.assignments {
            if !valid_nodes.contains(owner) && !owner.is_unassigned() {
                errors.push(format!(
                    "partition {pid} assigned to unknown node {owner}"
                ));
            }
            *per_node.entry(*owner).or_insert(0) += 1;
        }
...
mod tests {
    use super::*;
    use crate::constellation::discovery::{NodeMetadata, NodeState};
    ...
    fn test_initial_assignment_basic() {
        ...
        for (_, owner) in &plan.assignments {
    ...
    fn test_weighted_distribution() {
        ...
        assert!(node1_count > node2_count, "node1={node1_count}, node2={node2_count}");
        assert!(
            node1_count > 50,
