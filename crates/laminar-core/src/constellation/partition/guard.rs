...
impl PartitionGuard {
    ...
    pub fn update_cached_epoch(&self, new_epoch: u64) {
        self.cached_current_epoch.store(new_epoch, Ordering::Release);
    }
...
impl ConditionalPut {
    ...
    pub fn to_metadata(&self) -> Vec<(String, String)> {
        vec![
            (
                "x-amz-meta-epoch".into(),
                self.expected_epoch.to_string(),
            ),
            (
...
mod tests {
    ...
    fn test_conditional_put_metadata() {
        ...
        assert!(meta.iter().any(|(k, v)| k == "x-amz-meta-epoch" && v == "10"));
        assert!(meta.iter().any(|(k, v)| k == "x-amz-meta-writer-node" && v == "3"));
    }
