//! Raft coordinator wrapper for constellation metadata consensus.
//!
//! Wraps `openraft::Raft` to provide a higher-level API for reading
//! and writing constellation metadata through the Raft log.
...
/// Raft coordinator for constellation metadata.
///
/// Wraps `openraft::Raft` and the `ConstellationMetadata` state machine
/// to provide typed read/write operations.
pub struct RaftCoordinator {
    /// The local node ID.
    pub node_id: NodeId,
    // openraft::Raft instance and state machine will be added in F-COORD-001
}

impl RaftCoordinator {
    /// Create a new Raft coordinator (placeholder — fully implemented in F-COORD-001).
    #[must_use]
