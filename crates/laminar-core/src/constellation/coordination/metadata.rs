//! Constellation metadata state machine for Raft consensus.
//!
...
/// Log entries applied to the constellation metadata state machine.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub enum MetadataLogEntry {
    ...
    UpdateNodeState {
        ...
        node_id: NodeId,
        /// The new state.
        state: crate::constellation::discovery::NodeState,
    },
...
/// The Raft state machine for constellation metadata.
///
...
pub struct ConstellationMetadata {
    ...
    pub node_states: HashMap<u64, crate::constellation::discovery::NodeState>,
    /// Node ID → RPC address.
    ...
    pub fn apply(&mut self, entry: &MetadataLogEntry) {
        match entry {
            ...
            MetadataLogEntry::RegisterNode {
                ...
                self.node_states
                    .insert(node_id.0, crate::constellation::discovery::NodeState::Joining);
                self.node_addresses
                    .insert(node_id.0, rpc_address.clone());
            }
    ...
    pub fn active_node_count(&self) -> usize {
        self.node_states
            ...
            .filter(|s| {
                matches!(
                    s,
                    crate::constellation::discovery::NodeState::Active
                        | crate::constellation::discovery::NodeState::Joining
                )
            })
            .count()
    }
...
mod tests {
    use super::*;
    use crate::constellation::discovery::NodeState;

    #[test]
    fn test_metadata_new() {
        let meta = ConstellationMetadata::new();
    ...
    fn test_assign_partition() {
        let mut meta = ConstellationMetadata::new();
    ...
    fn test_register_and_deregister_node() {
        ...
        meta.apply(&MetadataLogEntry::DeregisterNode {
            node_id: NodeId(1),
        });

        assert!(meta.node_states.get(&1).is_none());
        let owner = meta.partition_owner(0).unwrap();
        assert!(owner.node_id.is_unassigned());
    }
