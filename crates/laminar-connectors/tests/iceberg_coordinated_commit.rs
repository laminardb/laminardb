...
use laminar_connectors::connector::{
    ...
    CoordinatedCommitter, SinkConnector,
};
...
async fn inspect(table: &str) -> (usize, usize) {
    let cfg = IcebergSinkConfig::from_config(&config(table)).unwrap();
...
async fn designated_committer_one_snapshot_from_many_writers() {
    ...
    for w in 0..3i64 {
        let mut writer = open_sink(&table).await;
    ...
    let mut committer = open_sink(&table).await;
